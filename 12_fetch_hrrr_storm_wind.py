...
Usage:
    ...
    python 12_fetch_hrrr_storm_wind.py --timeline-only --timeline dec2022
    python 12_fetch_hrrr_storm_wind.py --timeline-only --timeline all
"""
...
CURATED_TIMELINES = {
    ...
    "dec2022": {
        "name": "December 2022 Pre-Christmas Windstorm",
        "start": "2022-12-22 18:00",
        "end": "2022-12-24 11:00",
        "precip_type": "rain",
        "timezone_note": (
            "UTC (13:00 Dec 22 through 06:00 Dec 24 EST); covers the approach, "
            "Thursday-night onset, Friday damaging winds, and Saturday departure"
        ),
        "observed_reference": {
            "event_id": "2022122218",
...
def build_timeline(key, cfg):
    ...
    return {
        ...
        "observed_reference": cfg.get("observed_reference"),
        "frames": frames,
    }


def _read_existing_timeline_data():
    """Load reviewed storms so adding one timeline does not erase the others."""
    if not TIMELINE_OUT_JS.exists():
        return None
    assignment = "window.CONNECTICUT_STORM_TIMELINES = "
    text = TIMELINE_OUT_JS.read_text()
    if assignment not in text:
        raise ValueError(f"Cannot parse existing timeline file: {TIMELINE_OUT_JS}")
    payload = text.split(assignment, 1)[1].strip()
    if payload.endswith(";"):
        payload = payload[:-1]
    existing = json.loads(payload)
    grid = existing.get("grid", {})
    if grid.get("n_lat") != len(TARGET_LATS) or grid.get("n_lon") != len(TARGET_LONS):
        raise ValueError("Existing timeline grid does not match the current Connecticut grid")
    return existing


def write_timeline_data(keys):
    keys = list(keys)
    unknown = [key for key in keys if key not in CURATED_TIMELINES]
    if unknown:
        raise KeyError(f"Unknown curated timeline(s): {', '.join(unknown)}")

    # A single-storm update preserves the other reviewed HRRR timelines. `all`
    # rebuilds every HRRR timeline declared above. Entries owned by a separate
    # generator (for example the explicitly labeled July 2026 reconstruction)
    # are preserved in either mode rather than silently erased.
    existing = _read_existing_timeline_data()
    existing_storms = existing.get("storms", {}) if existing else {}
    external_storms = {
        key: storm for key, storm in existing_storms.items()
        if key not in CURATED_TIMELINES
    }
    storms = {}
    if set(keys) != set(CURATED_TIMELINES):
        storms.update({
            key: storm for key, storm in existing_storms.items()
            if key in CURATED_TIMELINES
        })
    for key in keys:
        storms[key] = build_timeline(key, CURATED_TIMELINES[key])
    storms = {
        **{key: storms[key] for key in CURATED_TIMELINES if key in storms},
        **external_storms,
    }
    ...
    header = (
        ...
        "// Separately generated, explicitly labeled reconstructions are preserved.\n"
    ...
    frame_count = sum(len(storm["frames"]) for storm in storms.values())
    print(
        f"\nWrote {TIMELINE_OUT_JS} "
        f"({len(storms)} reviewed storms, {frame_count} hourly frames)"
    )
...
def parse_args():
    ...
    parser.add_argument(
        "--timeline",
        choices=["all", *sorted(CURATED_TIMELINES)],
        help="Build one curated hourly timeline, or rebuild all reviewed timelines.",
    )
    parser.add_argument(
        "--timeline-only",
        action="store_true",
...
def main():
    ...
    if not args.timeline_only:
        write_snapshot_data()
    if args.timeline:
        keys = list(CURATED_TIMELINES) if args.timeline == "all" else [args.timeline]
        write_timeline_data(keys)
