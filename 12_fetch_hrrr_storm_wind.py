...
def write_timeline_data(keys):
    ...
    # A single-storm update preserves the other reviewed timelines. `all`
    # rebuilds everything from the declared source windows for reproducibility.
    existing = None if set(keys) == set(CURATED_TIMELINES) else _read_existing_timeline_data()
    storms = {}
    if existing:
        storms.update({
            key: storm for key, storm in existing.get("storms", {}).items()
            if key in CURATED_TIMELINES
        })
    for key in keys:
        storms[key] = build_timeline(key, CURATED_TIMELINES[key])
    storms = {key: storms[key] for key in CURATED_TIMELINES if key in storms}
