...
| **Used by** | The interactive uses each real HIFLD point as a territory anchor. Synthetic feeder and lateral routes begin at the nearest mapped road node and follow the cached OpenStreetMap road graph described below. Map tooltips show real name + voltage. **The grid auto-builds on page load** so real names appear immediately without the user having to click Generate first. Substations are also the territory units for the crew-stickiness toggle. |
...
### Road-constrained synthetic feeder geometry

`data/road_grid.json` is generated by `build_road_grid_extract.py` from a
Connecticut OpenStreetMap road extract. For each HIFLD substation, the builder
uses the nearest road node, selects outward shortest paths in angular sectors
as synthetic feeders, and grows synthetic laterals along eligible local-road
branches. The browser and calibration harness match this asset to substations
by source-array position, not by the non-unique HIFLD name. These paths improve
geographic plausibility but are not observed utility circuits, conductor
records, protective-device boundaries, or switching topology. OpenStreetMap
data are licensed under ODbL 1.0.
