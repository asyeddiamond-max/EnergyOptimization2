/*
 * Bridges outage_location_model.js output into the restoration scheduler by
 * attaching per-outage metadata (critical / tree-blocked / flood / callback /
 * switching / underground) without ever moving or removing a placed outage.
 *
 * Author: Alex Luo (@alexl1239) -- original design and implementation,
 *   feature/outage-location-simulator.
 *
 * The physics constants and per-attribute logic are ported from the existing
 * scheduler in 03_grid_simulation.html (trimAgeMult, EVERSOURCE_CT
 * restoredUnder5min = 0.42, flood-corridor distance), so behaviour matches the
 * pre-existing model.
 */
