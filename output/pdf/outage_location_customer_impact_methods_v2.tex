...
The inputs are hourly NOAA High-Resolution Rapid Refresh (HRRR) wind and precipitation fields, 2020 Census population, and synthetic feeders and laterals anchored at public substation coordinates \citep{noaahrrr,census2020,hifld}. Raster calculations use a regular $41\times65$ Connecticut grid. Network lines remain vector geometry and are evaluated against that grid.
...
Every feeder and lateral is divided by chainage into candidates no longer than 0.25 km. Composite midpoint quadrature, also using a maximum spacing of 0.25 km, gives

\begin{equation}
  W_s=\lambda_{\mathrm{type}(s)}
  \sum_t\int_s I(x,t)\,d\ell,
  \qquad
  \lambda_{\mathrm{feeder}}=0.10,
  \quad
...
\textbf{6. Turn feeders and laterals into directed trees.}
A feeder coordinate list is interpreted from its substation toward its endpoint. A lateral is interpreted from its feeder attachment outward. Each segment records one parent and zero or more children. A lateral's first segment is attached to the feeder segment at the recorded or inferred attachment chainage. Attachments farther than 0.05 km from the feeder are rejected. The statewide network is therefore a rooted forest: one directed tree for each feeder root.
...
\begin{equation}
  W_{sg}=\frac{0.75W_s}{G_s}.
  \label{eq:group-weight}
\end{equation}

Thus all small groups on the segment together have 75 percent of the segment's network-failure weight, regardless of how many groups it contains.
...
Candidates are considered from largest key downward. Each network segment has a preorder subtree interval $[a_s,b_s]$. In a rooted tree, two network failures overlap when their intervals intersect:
...
Feeder susceptibility, candidate length, and total small-group weight were selected using aggregate National Grid D.P.U. 24-41 storm-job size bins supplied for this project \citep{dpu2441}. The fitting objective was the mean total-variation distance between simulated and target job shares across five calibration seeds. Five additional seeds were held out from parameter selection. The accepted settings produced held-out mean job-share total variation 0.1007, maximum absolute bin error 0.0578, and overflow job share 0.0025.

These are held-out stochastic seeds on the same synthetic calibration network, not validation on an independent utility topology or storm. The D.P.U. target influences the distribution of modeled job sizes; it does not directly assign a customer count to an outage.
...
\begin{itemize}[leftmargin=1.35em,itemsep=0.02em,topsep=0.1em,parsep=0pt]
  \item Substation locations are real, but feeders and laterals are synthetic.
  ...
  \item A failure location is sampled within a 0.25 km segment, while its size uses that segment's complete downstream total.
